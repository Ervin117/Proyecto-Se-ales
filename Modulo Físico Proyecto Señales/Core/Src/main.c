...
uint16_t adc_values[2]; //Arreglo para almacenar las conversiones del ADC
...
// Flag que indica que han pasado 10 ms (controlada por el timer)
volatile uint8_t flag_1ms = 0;

uint8_t id_cof;

float coefientes[9]; //Número recibido por UART

// Variables del sistema
float x_n = 0, x_n_1 = 0, x_n_2 = 0, x_n_3 = 0, x_n_4 = 0;
float y_n = 0, y_n_1 = 0, y_n_2 = 0, y_n_3 = 0, y_n_4 = 0;

// Variables para el DAC
#define VALOR_MAX 4095
uint16_t cont = 0;

uint16_t freq_timer = 4095; //En Hz
uint32_t timer_clk;
uint32_t pclk;
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  // Inicia el timer en modo interrupción para generar eventos cada 10 ms
